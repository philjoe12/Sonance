...
def validate_config(cfg: UpmixConfig) -> None:
    numeric_fields = {
...
class StreamingUpmixer:
    ...
    Input and output sample format are signed int16.
    Input frame shape is interleaved stereo: [L, R, L, R, ...].
    Output frame shape is interleaved 5.1: [L, R, C, LFE, Ls, Rs, ...].
    """

    def __init__(self, sample_rate: int, config: UpmixConfig | None = None) -> None:
        if sample_rate <= 0:
        ...
        delay_samples = max(1, int((self.config.surround_delay_ms / 1000.0) * self.sample_rate))
        self._delay_line = [0.0] * delay_samples
        self._delay_index = 0

        dt = 1.0 / self.sample_rate
        rc = 1.0 / (2.0 * 3.141592653589793 * self.config.lfe_lowpass_hz)
    ...
    def process_interleaved_stereo(self, stereo_samples: InterleavedStereoSamples) -> list[int]:
        ...
        for i in range(0, len(stereo_samples), 2):
            l = float(stereo_samples[i])
            r = float(stereo_samples[i + 1])
            ...
            self._lfe_state = _one_pole_lowpass(mono, self._lfe_state, self._alpha)
            lfe = cfg.lfe_gain * self._lfe_state

            delayed_side = self._delay_line[self._delay_index]
            self._delay_line[self._delay_index] = side
            self._delay_index = (self._delay_index + 1) % len(self._delay_line)
...
def upmix_stereo_wav_to_5_1(input_path: str, output_path: str, config: UpmixConfig | None = None) -> None:
    ...
    processor = StreamingUpmixer(sample_rate=framerate, config=cfg)
    out = processor.process_interleaved_stereo(samples)
